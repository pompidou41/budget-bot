...
- received: для перевода — сколько пришло на toAccount в его валюте, если названо (при обмене валют обязательно). Если вместо этого назван курс обмена ("100 евро налички в лари по 3,1") — received = amount, пересчитанный по этому курсу, округлённый до сотых. Иначе 0.
...
- manualRate: только если назван курс валюты счёта account к доллару именно для этой операции (для RUB — рублей за 1 USD). Курс обмена между двумя другими валютами (евро → лари) сюда не пиши — он уходит в received. Иначе 0.
