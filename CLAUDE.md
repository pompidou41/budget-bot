...
**MCP (Model Context Protocol)** — это система для подключения внешних сервисов к Claude Code. В этом проекте используется для расширения возможностей IDE при работе с кодом.

### Доступные MCP-серверы

| Сервер | Тип | Назначение |
|--------|-----|-----------|
| **context7** | HTTP | Контекстная интеграция для Claude Code |

### Где MCP работает

MCP доступен во всех средах Claude Code:
- **Claude Code IDE** — полная поддержка ✓
- **Claude Code на веб** — полная поддержка ✓
- **CLI контексты** — полная поддержка ✓

### Конфигурация

MCP конфигурируется через два файла в `.claude/`:

1. **`.mcp.json`** — определяет доступные MCP-серверы
   - context7 настроен как HTTP-сервер
   - Требует `CONTEXT7_API_KEY` для аутентификации

2. **`.claude/settings.local.json`** — активирует MCP-серверы
   - `enabledMcpjsonServers`: явно включает context7
   - `enableAllProjectMcpServers: true`: автоматически включает новые серверы из `.mcp.json`

**Примечание:** MCP — это инструмент разработки Claude Code и не интегрирован в само приложение бота. Боту не требуется MCP для работы.
