...
```typescript
type DraftView =
  | { kind: 'card' } // карточка: Сохранить / Сумма / Счёт / Категория / Дата / Тип / Разовая / Комментарий
  | { kind: 'pick'; picker: 'acc' | 'to' | 'cat' | 'sub' | 'date' | 'type' }
  | { kind: 'input'; field: 'amount' | 'received' | 'rate' | 'comment' | 'date'; since: number };
```
...
- Сумма с карточки: «💰 Сумма» (`d:<id>:amount`) открывает тот же `input`-вид, что и шаг мастера. Подсказка называет
  валюту счёта, поэтому введённое число снимает и вопрос о валюте — `mentionedCurrency` сбрасывается в `null`.
  У перевода кнопка правит отправленную сумму; полученная сумма при этом не пересчитывается.
- Обмен валют (перевод между счетами в разных валютах, `isExchange` в `domain/exchange.ts`): на карточке
  «💱 Курс» (`d:<id>:rate`) и «📥 Пришло» (`d:<id>:received`). Курс — только способ заполнить «Получено» (F):
  `received = amount × курс` (или `÷`), в «Курс (ручной)» (K) он **не** пишется — K это курс валюты счёта к USD.
  Направление котировки — как курс говорят вслух: базой идёт более дорогая валюта (3,1 GEL за 1 EUR, 90 RUB за 1 EUR);
  «дороговизна» берётся из J/K листа «Счета» (`balanceUsd / balance`), без остатков — за 1 единицу валюты отправления.
  Карточка показывает получившийся курс строкой `💱 1 EUR = 3,1 GEL`.
