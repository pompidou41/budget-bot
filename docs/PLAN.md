...
### v2.8 — контекст в разборе, комментарий на карточке, заметки для ИИ

- [x] Кнопка «💬 Комментарий» на карточке черновика; очистка только явной кнопкой, «Назад» не стирает
- [x] `analytics/context.ts` — сводки или живые операции, поток денег, места и люди, обязательные платежи, лента по дням
- [x] Недельное «обычно» без сводок, при нехватке живых недель — оценка из месяца; покупки и чек со сводками не сравниваются
- [x] Итоги распределения денег считаются заранее; колонка «обычно к N-му дню месяца»
- [x] Обязательные платежи по таксономии таблицы (статистика на сводках не находила ничего)
- [x] Алиасы → заметки для ИИ свободным текстом, старые переносятся автоматически
- [x] Три прогона разбора на реальных данных (неделя и месяц) с исправлением найденного
- [x] Голос равен тексту везде: ответ на шаг мастера, комментарий, заметка для ИИ, вопрос (`voiceAsText`)
- [ ] Проверка на dev-боте: кнопка комментария, экран заметок, кнопки `s:al` в старых сообщениях, голос в шаге ввода
